...
class JobCategory extends React.Component<Props, State> {
  ...
  handleSuggestionsFetchRequested = ({ value }: SuggestionsFetchRequestedParams) => {
    this.setState({
      suggestions: getSuggestions(value, this.props.industries),
    })
  }
  ...
  handleChange = (event: any, { newValue }: ChangeEvent) => {
    ...
    const selectedItem = this.props.industries.find(item => item.label === newValue) || null
